...
MASE $=0.121$ y BiTCN sobre diferencia $0.124$ (Tabla~\ref{tab:global_deep})—, mejor que
CatBoost y SARIMA. Aun así, \textbf{ningún modelo profundo global supera por sí solo a
los parsimoniosos} ETS y Theta ($0.118$). El valor del aprendizaje profundo en este
problema aparece \textit{en conjunto}: la mediana de \{BiTCN, ETS, Theta\} obtiene
MASE $=0.111$, el mejor resultado del estudio en FAD —marginalmente por debajo de la
combinación puramente estadística ($0.1115$) y del oráculo de selección ($0.112$)—.
\textbf{En la tabla DFF el resultado es claramente más favorable al aprendizaje
profundo}: en estas series más cortas, donde el entrenamiento global rinde más al prestar
señal entre series, \textbf{BiTCN global supera por sí solo a todos los modelos
estadísticos y de árboles} (MASE $=0.088$ frente a $0.100$ de SARIMA y $0.113$ de
CatBoost), siendo el único caso del estudio en que un modelo profundo es el mejor
absoluto; combinarlo con los estadísticos no lo mejora. La lectura honesta es que el
aprendizaje profundo \textit{sí} aporta valor a este dominio, pero su ventaja depende del
régimen de datos: marginal y solo en conjunto sobre las series largas de FAD —donde la
parsimonia, sola, es difícil de superar—, y decisiva sobre las series cortas de DFF,
donde el entrenamiento global vence a los modelos clásicos. El factor determinante no es
la capacidad del modelo sino entrenarlo de forma global y sobre la primera diferencia.

\begin{table}[H]
\centering
\caption[Modelos profundos globales (FAD)]{Modelos profundos entrenados de forma global
sobre las 25 series FAD familiares (mejor de las variantes nivel/diferencia por modelo).
MASE de hold-out; el listón es ETS/Theta $=0.118$. La mediana \{BiTCN, ETS, Theta\}
($0.111$) es el mejor sistema del estudio.}
\label{tab:global_deep}
\begin{tabular}{lcc}
\hline
\rowcolor{uacjblue!15}
\textbf{Modelo global} & \textbf{Variante} & \textbf{MASE hold-out} \\ \hline
PatchTST & nivel & 0.121 \\
BiTCN & diferencia & 0.124 \\
TiDE & diferencia & 0.132 \\
KAN & nivel & 0.133 \\
TimesNet & nivel & 0.140 \\
N-HiTS & diferencia & 0.150 \\
TimeMixer & diferencia & 0.157 \\ \hline
\rowcolor{uacjyellow!25}
Mediana \{BiTCN, ETS, Theta\} & ensamble & \textbf{0.111} \\ \hline
\end{tabular}
\end{table}
...
hallazgo central es claro y, a la vez, contrario a la intuición de que "más capacidad
es mejor": para series de fechas de prioridad —cortas, con tendencia fuerte,
estacionalidad casi nula y retrogresiones ocasionales—, \textbf{los modelos
parsimoniosos ganan}. ETS amortiguado y Theta encabezan FAD (MASE $=0.117$), CatBoost
sobre deltas encabeza DFF ($0.116$), y todos mejoran al naïve estacional por un margen
amplio. Las redes profundas entrenadas localmente (LSTM, DeepAR, TFT) caen por debajo
del naïve: con 125 a 290 observaciones por serie su capacidad se traduce en
sobreajuste, no en exactitud. Tres resultados matizan la respuesta a "¿conviene la
...
adicional en FAD. Y un cuarto resultado, el más favorable al aprendizaje profundo: (iv)
entrenado de forma \textbf{global} sobre el panel y sobre la primera diferencia, el mismo
tipo de red que fracasaba localmente se vuelve competitivo en FAD (BiTCN MASE $=0.124$) y
\textbf{el mejor modelo absoluto en DFF} (BiTCN $=0.088$, por debajo de SARIMA $0.100$ y
CatBoost $0.113$). La conclusión, por tanto, es matizada: la IA \textit{sí} aporta valor,
y su forma ganadora depende del régimen de datos —la parsimonia es difícil de superar en
las series largas de FAD, mientras que el aprendizaje profundo global vence en las series
cortas de DFF—; el factor determinante no es la capacidad del modelo sino entrenarlo de
forma global y sobre incrementos.
