...
\hline
\rowcolor{uacjblue!15}
\textbf{Familia} & \textbf{Modelos} & \textbf{Rol} \\ \hline
...
Lineales/profundos \cita{4}\cita{5}\cita{9} & DLinear, NLinear, RLinear, TiDE, & No linealidad y capacidad alta \\
 ...
 & N-BEATS, N-HiTS, TFT & \\
Foundation \textit{(zero-shot)} & Chronos & Transferencia sin entrenamiento \\ \hline
\end{tabular}
\end{table}
...
coherente con su robustez ante muestras pequeñas. El ganador por serie vuelve a
...
\hline
\rowcolor{uacjblue!15}
\textbf{Modelo} & \textbf{MASE sel.} & \textbf{MASE hold-out} & \textbf{sMAPE sel.} & \textbf{sMAPE hold-out} & \textbf{$n$} \\ \hline
CatBoost & 0.116 & 0.113 & 0.24 & 0.25 & 25 \\
ETS (damped) & 0.117 & 0.106 & 0.23 & 0.23 & 25 \\
...
DFF, en cambio, ninguna combinación de modelos por serie mejora a sus miembros. El cota de un selector perfecto (oráculo)
...
conocido sino BiTCN. \textit{Confirmación multi-semilla:} para caracterizar la estabilidad,
ambos resultados se reentrenaron con \textbf{cinco semillas independientes}, variando tanto
...
de los modelos clásicos}: su CRPS es de $22.2$ días en DFF y $31.5$ en FAD, frente a los
...
con \textit{inferencia conforme adaptativa} (Adaptive Conformal Inference), que ajusta el
...
intervalo al $95\,\%$ subió de $0.77$ a $0.95$ en DFF y de $0.89$ a $0.92$ en FAD, clavando
...
evaluado en modo zero-shot sobre el panel. Ninguna rompe la frontera de exactitud puntual
en FAD: el \textit{stacking} y el meta-aprendizaje sobreajustan con apenas $25$ series, y
TabPFN-TS alcanza un MASE de $0.1155$ —a la altura del listón parsimonioso ($0.117$) y del
mejor modelo profundo ($0.112$), pero sin descender hacia el oráculo de selección perfecta
($0.112$)—. Que un modelo preentrenado, \textbf{sin ajuste alguno a estos datos}, iguale a
...
\hline
\rowcolor{uacjblue!15}
\textbf{Modelo / configuración} & \textbf{Técnica} & \textbf{MASE hold-out} \\ \hline
PatchTST & global, nivel & 0.121 \\
BiTCN & global, diferencia & 0.123 \\
TiDE & global, diferencia & 0.132 \\
BiTCN & + agrupado + norma por serie & 0.116 \\
AutoTiDE & + optimización (Optuna) & 0.115 \\ \hline
\rowcolor{uacjyellow!25}
AutoBiTCN & + optimización (Optuna) & \textbf{0.112} \\ \hline
ETS / Theta \textit{(listón)} & estadístico, por serie & \textit{0.117} \\ \hline
\end{tabular}
\end{table}
...
\hline
\rowcolor{uacjblue!15}
\textbf{Modelo} & \textbf{EB FAD (MASE sel.)} & \textbf{EB DFF (MASE sel.)} \\ \hline
ETS (damped) & 0.228 & 0.133 \\
Theta & 0.248 & 0.129 \\
ARIMA & 0.278 & 0.141 \\
...
Naïve estacional & 1.970 & 1.174 \\ \hline
\end{tabular}
\end{table}
...
seis familias bajo un protocolo de validación temporal sin fuga de información. El
primer hallazgo, contrario a la intuición de que "más capacidad es mejor": evaluados
\textit{por serie}, para fechas de prioridad —cortas, con tendencia fuerte, estacionalidad
casi nula y retrogresiones ocasionales—, \textbf{los modelos parsimoniosos ganan}. ETS
amortiguado y Theta encabezan FAD (MASE $=0.117$), CatBoost sobre deltas encabeza DFF
($0.116$), y todos mejoran al naïve estacional por un margen amplio. Las redes profundas
entrenadas \textit{localmente} (LSTM, DeepAR, TFT) caen por debajo del naïve: con 125 a
290 observaciones por serie su capacidad se traduce en sobreajuste, no en exactitud. Este
cuadro, sin embargo, se revierte cuando las redes se entrenan como corresponde a un panel
(punto~(iv) más abajo). Tres resultados matizan la respuesta a "¿conviene la
IA?": (i) un \textit{foundation model} preentrenado evaluado sin ajuste (Chronos) supera
a todas las redes entrenadas localmente, mostrando que la transferencia vence a la
...
acerca a los parsimoniosos sin superarlos; y (iii) ningún conjunto amplio mejora al
mejor modelo, y solo una combinación curada de los tres modelos fuertes araña un $5\,\%$
adicional en FAD. Y un cuarto resultado, el más contundente: (iv) entrenado de forma
...
el mismo tipo de red que fracasaba localmente pasa a ser \textbf{competitivo con los
modelos parsimoniosos, y los supera donde los datos escasean}: BiTCN logra MASE $=0.090$ en
DFF —$\sim$15\,\% por debajo del hold-out del mejor parsimonioso de cobertura completa
(ETS $0.106$)— mientras que AutoBiTCN \textbf{empata} con ETS/Theta en FAD ($0.112$ frente
a $0.117$). Una validación con cinco semillas precisa esta lectura: en DFF la ventaja es
\textbf{holgada y estable} (media $0.090\pm0.0003$, intervalo de confianza enteramente bajo
el listón), y en FAD es un \textbf{empate estadístico} (media $0.112\pm0.008$, intervalo
$[0.102,\,0.122]$ que contiene el listón $0.117$). La conclusión, por tanto, depende del régimen: \textbf{el aprendizaje
profundo gana en DFF (series cortas, donde el entrenamiento global presta señal entre
series) e iguala a la parsimonia en FAD (series largas, donde un modelo simple ya es
óptimo)}. Y cuando aporta, la ventaja \textbf{no proviene de la capacidad del modelo sino de
cómo se entrena} —global, sobre incrementos, con escalado por serie y regularización
ajustada—; sin esos ingredientes las mismas redes caen por debajo del naïve, y modelos de
gran nombre como DeepAR fallan. La respuesta a ``¿conviene la IA?'' es \textit{matizada}:
sí en series cortas entrenada como un panel; en series largas, un modelo estadístico simple
la iguala a una fracción del costo.

Este resultado no es una limitación del trabajo sino un hallazgo validado: documenta,
con un protocolo reproducible y libre de fuga, qué familia de modelos conviene a cada
régimen de este dominio y por qué. La cobertura se extendió de las categorías familiares
a las basadas en empleo (EB), donde el patrón se replica (parsimonia y árboles al frente,
heterogeneidad por serie), confirmando que las conclusiones no son un artefacto de un
subconjunto de categorías. El sistema entrega pronósticos con intervalos de predicción al
95\,\% cuya cobertura empírica se verifica, y un marco reproducible de comparación que se
extenderá, como trabajo futuro, al tratamiento de los cambios de régimen
($C \leftrightarrow F \leftrightarrow U$) que en esta etapa quedan fuera del objetivo
predictivo.
