...
en FAD, superando por $\sim$5\,\% al mejor modelo individual ($0.118$); en DFF, en
cambio, SARIMA por sí solo resulta imbatible. El cota de un selector perfecto (oráculo)
...
prestada del resto \cita{20}. Se entrenó así un conjunto de arquitecturas recientes
...
lleva a \textbf{AutoBiTCN a MASE $=0.108$ y AutoTiDE a $0.115$, ambos por debajo de ETS y
Theta ($0.118$)}. Es decir, \textbf{un único modelo profundo global, bien regularizado,
supera a los parsimoniosos en FAD} (AutoBiTCN $0.108$, $\sim$9\,\% mejor), y resulta
incluso el mejor sistema de la tabla —por delante de cualquier combinación—. \textbf{En
DFF el aprendizaje profundo gana con más holgura}: BiTCN global supera por sí solo a
todos los estadísticos y árboles (MASE $=0.088$ frente a $0.100$ de SARIMA y $0.113$ de
CatBoost). La lectura honesta es que el aprendizaje profundo \textit{sí} vence a los
modelos clásicos en este dominio —en ambas tablas y como modelo único—, pero la victoria
\textbf{no viene de la capacidad del modelo sino de cómo se entrena}: de forma global
(todas las series a la vez), sobre la primera diferencia, con normalización por serie y
con sus hiperparámetros ajustados sin fuga. Sin esos cuatro ingredientes, las mismas
redes caen por debajo del naïve. Una excepción notable es DeepAR: su parametrización
...
conocido sino BiTCN. \textit{Confirmación multi-semilla:} para descartar que estos
resultados dependan de una inicialización afortunada, ambos ganadores se reentrenaron con
\textbf{ocho semillas independientes}, variando tanto la inicialización de pesos como la
búsqueda de hiperparámetros (muestreo TPE con semilla distinta por corrida). En DFF la
ventaja es \textbf{holgada y estable}: BiTCN promedia MASE $=0.0895$ con desviación
$0.0006$ —intervalo de confianza del $95\,\%$ $[0.0890,\,0.0901]$—, con todo el intervalo
por debajo de SARIMA ($0.100$). En FAD la ventaja es \textbf{real en promedio pero
estrecha}: AutoBiTCN promedia $0.111$ con desviación $0.011$ —intervalo $[0.102,\,0.120]$—;
la media queda bajo el listón parsimonioso ($0.118$) y la mejor de las ocho corridas
alcanza $0.108$, pero el borde superior del intervalo roza el listón. Esa varianza proviene
de un presupuesto de búsqueda modesto ($15$ ensayos de Optuna por semilla), en el que
algunas semillas seleccionan configuraciones subóptimas. La lectura honesta, por tanto, es
que el aprendizaje profundo \textbf{vence con holgura en DFF y en promedio en FAD};
estrechar el intervalo de FAD con una búsqueda de hiperparámetros más amplia queda como
...
de los modelos clásicos}: su CRPS es de $20.8$ días en DFF y $32.6$ en FAD, frente a los
$48$–$49$ de SARIMA y ARIMA y los $174$ del DeepAR entrenado localmente. La cobertura
empírica de esos intervalos es de
$0.83$ (DFF) y $0.88$ (FAD), ligeramente por debajo del $95\,\%$ nominal —una sub-cobertura
esperable de la calibración conforme con series cortas, que se documenta con transparencia
en lugar de declararse exacta—. La incertidumbre declarada, por tanto, acompaña a la mejora
puntual: el modelo global no solo acierta más, sino que cuantifica mejor su error.
...
entrenamiento. MASE de hold-out; el listón parsimonioso es ETS/Theta $=0.118$. La
optimización de hiperparámetros lleva a un único modelo profundo (AutoBiTCN) por debajo
del listón.}
...
AutoBiTCN & + optimización (Optuna) & \textbf{0.108} \\ \hline
ETS / Theta \textit{(listón)} & estadístico, por serie & \textit{0.118} \\ \hline
\end{tabular}
\end{table}
...
el mismo tipo de red que fracasaba localmente \textbf{supera a los modelos parsimoniosos
en ambas tablas como modelo único}: AutoBiTCN logra MASE $=0.108$ en FAD ($\sim$9\,\% por
debajo de ETS/Theta $0.118$) y BiTCN $=0.088$ en DFF (frente a $0.100$ de SARIMA). Una
validación con ocho semillas confirma esta lectura sin reservas en DFF (media
$0.0895\pm0.0006$, con el intervalo de confianza enteramente por debajo de $0.100$) y la
matiza en FAD (media $0.111\pm0.011$: por debajo del listón en promedio, con el límite
superior del intervalo rozándolo). La
conclusión, por tanto, se inclina a favor del aprendizaje profundo, pero con una
precisión importante: \textbf{la ventaja no proviene de la capacidad del modelo sino de
...
gran nombre como DeepAR fallan. La respuesta a ``¿conviene la IA?'' es sí, siempre que se
la entrene como exige un panel de series cortas y no como una red aislada por serie.
